...
class AuthIntegrationTest:
    ...
    def __init__(self, db_path: str = "gateway_with_auth.db"):
    ...
    def setup_database(self):
        """设置数据库"""
        print("�� 设置包含认证功能的数据库...")
        with open('database_with_auth.sql', 'r') as f:
            sql_content = f.read()
        try:
...
def main():
    ...
    try:
        ...
        print("\n🎉 认证系统集成测试完成！")
        print("📁 数据库文件: gateway_with_auth.db")
        print("💡 可以使用 SQLite 工具查看数据:")
        print("   sqlite3 gateway_with_auth.db")
